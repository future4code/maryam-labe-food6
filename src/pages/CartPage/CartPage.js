...
const CartPage = () => {
  ...
  const dataPlaceOrder = () => {
    if (
      ...
      toast.warn("Adicione um produto ao carrinho");
    } else if (cart.paymentMethod === "" && cart.products.length > 0) {
      toast.warn("Por favor, selecione um método de pagamento para prosseguir");
    } else {
      toast.warn(
        "Para prosseguir com a compra, adicione produtos ao carrinho e selecione um método de pagamento"
      );
    }
  };
  ...
  const totalCart = () => {
    ...
    for (const product of cartProducts) {
      item = cart.products.find((item) => item.id === product.id);
  ...
  const ContainerFoodCard =
    cartProducts &&
    cartProducts.map((product) => {
      const findId = cart?.products.filter((prod) => prod.id === product.id);
      return (
    ...
    <div>
      ...
      {actualRestaurant.id ? (
        <MainContainer>
          ...
          <Subtotal>
            <p>FRETE</p>
            <p>R$ {actualRestaurant.shipping},00</p>
          </Subtotal>
          <Total>
            <p>TOTAL</p>
